...
public class Lozinki {
    public static void main (String[] args) throws IOException {
        ...
        CBHT<String,String> mapa = new CBHT<>(2*N);
        for(int i=1;i<=N;i++){
            String imelozinka = br.readLine();
            String[] pom = imelozinka.split(" ");
            mapa.insert(pom[0],pom[1]);
        } // ime i pass se unasav n puta u mapu
        ...
        for(;;)
        {
            ...
            if(red.equals("KRAJ"))
                break;
            // delimo ga red na 2, t.e. ime password
            String[] delovi=red.split(" ");
            SLLNode<MapEntry<String,String>> temp= mapa.search(delovi[0]);
            // mapa trazi ime, ako ne najde(null) ili ime==pass nenajaven
            if(temp==null || !(temp.getElement().value.equals(delovi[1])))
            {
