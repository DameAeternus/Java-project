...
public class Cities {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Graph<String> graf = new Graph<String>(n, town);
        ...
        for (int i = 0; i < n_edges; i++) {
            ...
            yInfo = pom[3];
            graf.adjList[x].setInfo(xInfo);
            graf.adjList[y].setInfo(yInfo);
            weight = Float.parseFloat(pom[4]);
            graf.addEdge(x, y, weight);
        }
        String start = br.readLine();
        String end = br.readLine();
        br.close();
        int from = graf.searchIndex(start);
        int to = graf.searchIndex(end);
        float min_p = graf.min_path(from, to);
