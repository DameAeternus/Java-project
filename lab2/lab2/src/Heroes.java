...
public class Heroes {
    ...
    public static void startHeroesGame(SLL<Card> firstFriendCards, SLL<Card> secondFriendCards) {
        SLLNode<Card> first = firstFriendCards.getFirst();
        SLLNode<Card> najjaka = firstFriendCards.getFirst();
        SLLNode<Card> sredna = secondFriendCards.getFirst();
        while(first!=null)
        {
            if(first.element.damage()>najjaka.element.damage())
                najjaka=first;
            first=first.succ;
        }
        for(int i=1;i<3;i++)
        ...
        firstFriendCards.delete(najjaka);
        secondFriendCards.insertAfter(najjaka.element,sredna);
    }
