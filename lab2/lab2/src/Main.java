...
public class ArrayMeanValue {
    ...
    public static int brojDoProsek(Array<Integer> arr) {
        ...
        {
            sum+=arr.get(i); // 1+2+3+4+5+6 = 21/6=3.6
        }
        sum=sum/arr.getSize();
        int minimum=Math.abs(arr.get(0)-sum); // 2
        int br=0;
        for(int i=1;i<arr.getSize();i++)
        {
            if(minimum>Math.abs(arr.get(i)-sum))
            {
                minimum=Math.abs(arr.get(i)-sum);
                br=i;
            }
            if(minimum==Math.abs(arr.get(i)-sum)) // u slucaj pr 3.5 a ima broevi 3 i 4, 3 da ga zeme!
            {
                if(arr.get(br)>arr.get(i))
                {
                    minimum=Math.abs(arr.get(i)-sum);
                    br=i;
                }
