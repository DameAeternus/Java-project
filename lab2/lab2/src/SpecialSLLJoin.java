...
public class SpecialSLLJoin<E> {
    ...
    public SLL<E> specialJoin(SLL<E> list1, SLL<E> list2) {
        SLLNode<E> prv= list1.getFirst();
        SLLNode<E> vtor= list2.getFirst();
        SLL<E> lista= new SLL<E>();
        int br=1;
        while(prv!=null || vtor!=null)
        {
            if(prv==null){
                lista.insertLast(vtor.element);
                vtor=vtor.succ;}
            else if(vtor==null){
                lista.insertLast(prv.element);
                prv=prv.succ;
            }
            else if(br==1 && prv !=null)
            {
                lista.insertLast(prv.element);
                prv=prv.succ;
                br+=1;
            }
            else if(br==2 && prv!= null)
            {
                lista.insertLast(prv.element);
                prv=prv.succ;
                br+=1;
            }
            else if(br==3 && vtor!=null)
            {
                lista.insertLast(vtor.element);
                vtor=vtor.succ;
                br+=1;
            }
            else if(br==4 && vtor!=null)
            {
                lista.insertLast(vtor.element);
                vtor=vtor.succ;
                br=1;
